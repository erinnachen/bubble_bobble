...
export function gameOver2P(dino, dino2) {
  debugger;
  if (dino.lives === 0 || dino2.lives === 0 || dino.points >= 10000 || dino2.points >= 10000) { return true; }
  return false;
}
...
export function checkDinoBubbleCollisions(dino, bubbles, fruits, canvas, twoPlayer){
  bubbles.forEach(function(bubble){
    ...
    if (Collision.collision(bubble_collider, dino_receiver)){
      if (bubble.filled && bubble.status !== "popped") {
        ...
        if (twoPlayer) { fruit.floorHeight = 20; }
        fruits.push(fruit);
      }
...
export function drawWindups(windups, context, dino, floors) {
  windups.forEach(function(windup) {
